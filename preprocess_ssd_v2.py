...
def ssd_random_sample_patch(image, labels, polys, bboxes, ratio_list=[0.1, 0.3, 0.5, 0.7, 0.9, 1.], name=None):
    ...
    def sample_patch(image, labels, polys, bboxes, min_iou):
        with tf.name_scope('sample_patch'):
            ...
            if tf.logical_or(tf.less(roi_slice_range[2], 1), tf.less(roi_slice_range[3], 1)):
                image = tf.image.crop_to_bounding_box(image, roi_slice_range[0], roi_slice_range[1], roi_slice_range[2], roi_slice_range[3])
                labels = mask_labels
                polys = mask_polys / scale[..., :2]
                bboxes = mask_bboxes / scale

            return (image, labels, polys, bboxes)
    ...
    with tf.name_scope('ssd_random_sample_patch'):
        ...
        if sampled_min_iou < 1:
            return sample_patch(image, labels, polys, bboxes, sampled_min_iou)
...
def ssd_random_sample_patch_wrapper(image, labels, polys, bboxes):
    with tf.name_scope('ssd_random_sample_patch_wrapper'):
        ...
        image = tf.zeros([None, None, 3], dtype=tf.float32)
        ...
        index = 0
        max_attempt = 3
        for index in tf.range(3):
            if tf.random.uniform([], minval=0., maxval=1., dtype=tf.float32) < 0.5:
                image, polys, bboxes = ssd_random_expand(orig_image, orig_polys, orig_bboxes, tf.random.uniform([1], minval=1.1, maxval=4., dtype=tf.float32)[0])

            # Distort image and bounding boxes.
            image, labels, polys, bboxes = ssd_random_sample_patch(image, orig_labels, polys, bboxes, ratio_list=[-0.1, 0.1, 0.3, 0.5, 0.7, 0.9, 1.])

            valid_mask = check_bboxes(bboxes)
            cond = tf.cast(valid_mask, tf.int64)
            cond = tf.reduce_sum(cond)
            if cond >= 1:
