...
parser.add_argument('--image_dir', type=str, action='append', help='Image dirs')
parser.add_argument('--text_dir', type=str, action='append', help='Text dirs')
...
def scan_annotations(gt_dir, writer, image_fns):
    ...
    for gt_fn in os.listdir(gt_dir):
        spl = os.path.splitext(gt_fn.lower())
        if len(spl) != 2:
            continue
